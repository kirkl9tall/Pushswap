...
def main():
    ...
    for i in range(total_tests):
        ...
        min_moves_seen = min(max_moves_seen, moves)
        ...
        if moves > 5000:
            ...
            print(f"\nTest {i+1} exceeded  moves ({moves} moves)")
            print(f"Failing case saved to: {filename}")
    
    print("\n\nTesting completed!")
    print(f"Total tests run: {total_tests}")
    print(f"Number of cases exceeding the range: {failing_cases}")
    print(f"Maximum moves seen: {max_moves_seen}")
    print(f"min moves seen: {min_moves_seen}")
